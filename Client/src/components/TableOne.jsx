...
const TableOne = () => {
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/api/reviews/read`)
      .then((result) => {
  ...
  const handleApprove = (reviewId, email) => {
    fetch(`${baseUrl}/api/reviews/approve`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
      body: JSON.stringify({ id: reviewId, email: email }),
    })
      .then(response => {
        if (!response.ok) {
      ...
      .then(updatedReview => {
        console.log('Review status updated:', updatedReview);
  ...
  function handleDelete() {
    ...
    fetch(`${baseUrl}/reviews/delete/${localStorage.getItem("deletionId")}`, {
      method: 'DELETE'
    })
          ...
          {reviews.map((review) => (
            ...
            <div className="grid grid-cols-3 border-b border-stroke dark:border-strokedark sm:grid-cols-5">
              <div className="flex items-center justify-start gap-3 p-2.5 xl:p-5">
                ...
                <p className="hidden text-black dark:text-white sm:block min-w-[150px]">{review.email.substr(0, 20) + "..."}</p>
              </div>

              <div className="flex items-center justify-start p-2.5 xl:p-5 min-w-[290px]">
                <p className="text-black dark:text-white">{review.paragraph}</p>
              </div>

              <div className="flex items-center justify-center p-2.5 xl:p-5 min-w-[60px]">
                <p className="text-meta-3">{calculateAverage(review.infrastructureRatings, review.locationRatings, review.facultyRatings, review.educationRatings)}</p>
              </div>

              <div className="hidden items-center justify-start p-2.5 sm:flex xl:p-5 min-w-[190px]">
                <p className="text-black dark:text-white">{review.instiName.substr(0, 30) + "..."}</p>
              </div>

              <div className=" flex items-center justify-start p-2.5 sm:flex xl:p-5">
                <div className="flex items-center space-x-3.5">
                  <NavLink to="/editForm">
                    <button onClick={() => handleView(review._id)}
                      className="hover:text-[#14267d] text-[#2237a2] hover:scale-105">
                      <svg
                  ...
                  <button onClick={() => handleModal(review._id)}
                    className="hover:text-[#ff0000]  text-[#be2929] hover:scale-105">
                    <svg
                      className="fill-current"
                      width="18"
                      height="18"
                  ...
                  <button onClick={() => handleApprove(review._id, review.email)} className="hover:scale-105">
                    {/* Approve review */}
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" id="right"><path fill="none" stroke="#25b39e" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.01 7.755 5.297 12l8.691-6.991"></path></svg>
                  </button>
                </div>
              </div>
            </div>))
